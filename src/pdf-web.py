...
parse = st.selectbox("変換方法を選択してください", ("Text抽出", "HTML変換", "Markdown変換"))
...
if send_button and input_text and pdf_file:
    ...
    if parse == "Text抽出" or parse == "HTML変換":
        ...
        for page in PDFPage.get_pages(pdf_file, maxpages=max_pages, check_extractable=False):
        ...
        text = pymupdf4llm.to_markdown(Document(
            stream=pdf_file.getvalue()), pages=range(0, max_pages), margins=(0, 0, 0, 0))
    ...
    system_prompt = "documentの中にはPDFから抽出したテキストが格納されています。質問に回答してください。<document>" + \
        text + "</document>"
